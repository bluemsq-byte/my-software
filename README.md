...
- 全局采用半透明毛玻璃卡片和主题渐变背景。
- 歌曲行统一提供三点菜单，支持立即播放、下一首播放和加入队列。
- NAS 文件夹页增加文件夹概览和播放全部快捷入口。
- 设置页使用主题色块选择六种颜色主题。
- 按产品设计规范和 `design/sketches.html` 全量草图重做核心页面视觉。
- 主要页面使用透明顶栏、悬浮迷你播放器和统一的卡片层级。
- 16 页 Compose 草图代码已接入真实首页、网络、NAS、播放、列表、定时和设置页面。
- 底部导航、主题色、迷你播放器和播放页已使用设计令牌动态联动。
- 播放队列和播放位置支持持久化恢复，启动后不会自动发声。
- 首页增加继续播放卡片，支持从上次位置继续。
- 增加本地、全部 NAS 和播放列表的全局搜索。
- NAS 文件夹支持排序、多选、播放所选和批量加入队列。
- 本地音乐库增加专辑和歌手聚合视图。
- 定时播放失败会重试一次，仍失败则跳过并通知。
- 设置页支持无密码备份和恢复连接、播放列表、定时及主题。

## 安装包

- Release APK：`dist/audio-player-v1.8.0.apk`
- Debug APK：`dist/audio-player-v1.8.0-debug.apk`
